...
working_dir = '/home/furkankykc/workspace/rnaseq/expression/mice'
setwd(working_dir)
...
kallisto_names = c("length", "NCT_Rep1", "NCT_Rep2", "SLN_Rep1", "SLN_Rep2") 
names(kallisto_gene) = kallisto_names
names(kallisto_tran) = kallisto_names

sample_names = c("NCT_Rep1", "NCT_Rep2", "SLN_Rep1", "SLN_Rep2")
...
#1. Plot kallisto gene TPMs vs stringtie gene TPMs - Pick NCT_Rep1 data arbitrarily
stabvar = 0.1
NCT1_gene_data = data.frame(kallisto_gene[,"NCT_Rep1"], stringtie_gene[,"NCT_Rep1"], htseq_gene_counts[,"NCT_Rep1"])
names(NCT1_gene_data) = c("kallisto", "stringtie", "htseq")
p1 = ggplot(NCT1_gene_data, aes(log2(kallisto+stabvar), log2(stringtie+stabvar)))
p1 = p1 + geom_point()
...
p1 = p1 + labs(title = "NCT1 gene expression values [log2(value + 0.1) scaled]")

#2. Plot kallisto transcript TPMs vs stringtie transcript TPMs - Pick NCT_Rep1 data arbitrarily
# Indicate with the points whether the data are real transcripts vs. spike-in controls
NCT1_tran_data = data.frame(kallisto_tran[,"NCT_Rep1"], stringtie_tran[,"NCT_Rep1"])
names(NCT1_tran_data) = c("kallisto", "stringtie")
spikein_status=grepl("ERCC",tran_names)
p2 = ggplot(NCT1_tran_data, aes(log2(kallisto+stabvar), log2(stringtie+stabvar)))
p2 = p2 + geom_point()
p2 = p2 + geom_point(aes(colour = spikein_status)) 
p2 = p2 + xlab("Kallisto TPM") + ylab("StringTie TPM") + labs(colour = "SpikeIn Status")
p2 = p2 + labs(title = "NCT1 transcript expression values [log2(value + 0.1) scaled]")

#3. Plot stringtie transcript TPMs vs. stringtie transcript FPKMs - Pick NCT_Rep1 data arbitrarily
# Indicate with the points whether the data are real transcripts vs. spike-in controls
NCT1_tran_data2 = data.frame(stringtie_tran[,"NCT_Rep1"], stringtie_tran_fpkm[,"NCT_Rep1"])
names(NCT1_tran_data2) = c("stringtie_TPM", "stringtie_FPKM")
p3 = ggplot(NCT1_tran_data2, aes(log2(stringtie_TPM+stabvar), log2(stringtie_FPKM+stabvar)))
p3 = p3 + geom_point()
...
p3 = p3 + labs(title = "NCT1 transcript expression values [log2(value + 0.1) scaled]")
