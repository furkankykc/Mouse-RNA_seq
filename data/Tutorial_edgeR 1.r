...
mapping=read.table(args[1],header=FALSE, stringsAsFactors=FALSE,row.names=1)
# Read in count matrix
#rawdata=read.table("~/workspace/rnaseq/expression/mice/htseq_counts/gene_read_counts_table_all_final.tsv", header=TRUE, stringsAsFactors=FALSE, row.names=1)
rawdata=read.table(args[2],header=TRUE,stringsAsFactors=FALSE, row.names=1)
# Check dimensions
dim(rawdata)
...
# make class labels
class <- factor( c( rep("SLN",2), rep("NCT",2) ))
